"""Run a Python snippet in a fresh subprocess. Sandbox is cwd-only -- same
caveat as the bash tool (no isolation, runs as the same user)."""
import json
import os
import subprocess
import sys
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"error": f"timeout after {timeout}s"}))
        return 1
    except OSError as e:
