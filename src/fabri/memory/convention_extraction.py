    ...
    *,
    ...
    for candidate_index, raw_candidate in enumerate(candidates):
        ...
        if isinstance(raw_candidate, Mapping):
            ...
            # v1 allows exactly one effect class, so the model does not get to
            # author this field: five smoke rounds showed free-texted values
            # ("customer-evidence response mapping", ...) killing otherwise
            # perfect records at the gate. The engine sets the constant below;
            # action-bearing branch CONTENT is still rejected independently by
            # convention_quarantine_reason.
            candidate_with_scope.pop("effect_class", None)
