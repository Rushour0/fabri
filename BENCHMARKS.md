...
| **company memory & generational evolution** | Whether fabri's memory makes a *company* better or cheaper — a 60-arm memory-vs-control study plus immutable generational accumulation (gen-001 → gen-002). OpenAI-only, 3 roster companies. | **inconclusive — the evaluator is broken, not the agent.** The forbidden-term rubric is a substring match that false-positives on negations (flags the correct "no evidence a fix was deployed"); corrected, Reliability memory is 8/8 not 5/8 and gen-002 ties gen-001 on quality, cost neutral. The self-mining control isn't a no-memory baseline (0 clean control completions on 2/3 companies). Revenue Ops was a config failure (`max_tokens: 768` truncation; 768→2048 fix verified). Net: fix the instruments (negation-aware/LLM-judge rubric + true no-memory control) before any cost claim. See [qualitative analysis + 118 per-run logs](benchmarks/results/run-logs-2026-07-22/QUALITATIVE-ANALYSIS.md) and [consolidated report](benchmarks/results/company-generation-evolution-2026-07-22.md). **RESOLVED 2026-07-22 — instruments fixed and re-measured, see the row below** |
| **memory vs *true* no-memory control (fixed rubric)** | Re-run of the above after fixing all three defects: forbidden-side negation false-positives (now sentence-scoped), required-side substring false-negatives (now order-preserving proximity), and a control that still mined/retrieved within its own run (now `memory.mining_enabled`/`retrieval_enabled=False`, verified at **0 guidelines retrieved**). 36 arms, 3 companies × 6 replicas × {memory, control}, OpenAI-only. | **No significant memory effect — on quality or cost.** Support HQ and Reliability Labs tie at 100% corrected quality; Revenue Ops' apparent 100% vs 66.7% collapses to 4/4 vs 3/4 (one discordant pair, p=1.0) once paired on arms where both completed. Cost: memory cheaper in 2/5, 2/5, 0/4 clean pairs (p=1.0, 1.0, 0.125), every mean Δ positive (marginally *more* expensive). **The instrument was the confound in both directions** — the prior "memory hurts" headline and this study's own initial "+100pp memory wins" were both scoring artifacts. Spend $4.82/38 arms. See [report](benchmarks/results/memory-vs-true-control-2026-07-22.md) |
