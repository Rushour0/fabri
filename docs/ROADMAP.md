...
- **M3** • Retrieval-decision observability • Track M • **shipped 2026-07-07** • Emits one structured `retrieval` trace event per call with strategy, pool sizes, score ranges, drop counts, and final candidate inclusion reasons. The shipped implementation also fixed the dense-path `sparse_results` `NameError` and added retrieval trace coverage. This is the debug surface consumed by reports, benchmark rollups, and OTel export. Full design history: `docs/design/memory-observability-plan.md` (unit A).

- **M4** • Retrieval eval harness (the ground-truth gate) • Track M • **shipped 2026-07-07** • Deterministic, offline retrieval evaluation now runs Fabri's real retrieval path over a labeled 40-guideline/24-query fixture and reports recall@k, MRR, and precision@k without model credits. CI gates the measured floors, and `python -m fabri.benchmarks.retrieval_eval` compares strategies. The released hybrid result is recall@5 0.938 and MRR 0.844; current numbers and findings live in `BENCHMARKS.md`. Full design history: `docs/design/memory-observability-plan.md` (unit C).
...
- **X3** • General correctness eval harness • Track X • **partially shipped v0.18.5** • `company_setup_probe` now provides isolated fresh replicas, deterministic required/forbidden concept scoring, delegation completion checks, cost limits, and public aggregate/private trace separation for dynamic roster companies. Remaining scope: extract the scorer interface for arbitrary tasks and add exact-match plus optional LLM-as-judge implementations. Do not describe LongMemEval as proven until a result is published.
...
M3, M4, X1, and M6 are shipped. Continue M5 only behind the M4 regression
gate; complete X3 by extracting the roster probe's deterministic scorer before
adding an LLM judge. X2 stays on the safety track.
